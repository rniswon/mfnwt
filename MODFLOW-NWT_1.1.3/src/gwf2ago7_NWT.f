      ...
      END TYPE
      TYPE(GWFAGOTYPE), SAVE:: GWFAGODAT(10)
      END MODULE GWFAGOMODULE


      SUBROUTINE GWF2AGO7AR(IN,IUNITNWT,IGRID)
C     ******************************************************************
...
C1------IDENTIFY PACKAGE AND INITIALIZE NWELLS.
    ...
    1 FORMAT(1X,/1X,'AGO -- AGO PACKAGE FOR NWT VERSION 1.1.3, ',
     1' 0/10/2017 INPUT READ FROM UNIT ',I4)
      NWELLS=0
      NNPWEL=0
      IUNITRAMP=IOUT
C
...
C
      CALL PARSEAGOPTIONS(In, Iout, LINE, IUNITNWT)
C
...
C4------ALLOCATE SPACE FOR THE WELL DATA.
      ...
      IF(MXACTW.LT.1) THEN
         WRITE(IOUT,17)
   17    FORMAT(1X,
     1'Deactivating the AGO Package because MXACTW=0')
         IN=0
      END IF
...
C
C6------RETURN
      CALL SGWF2AGO7PSV(IGRID)
      RETURN
      END SUBROUTINE
C
      SUBROUTINE PARSEAGOPTIONS(IN,IOUT,line,IUNITNWT)
C     ******************************************************************
...
C
C
      SUBROUTINE GWF2AGO7RP(IN,KPER,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
      USE GLOBAL,       ONLY:IOUT,NCOL,NROW,NLAY,IFREFM,IUNIT
      USE GWFAGOMODULE, ONLY:NWELLS,MXWELL,NWELVL,IPRWEL,NPWEL,IWELPB,
...
C6------RETURN
      RETURN
      END
      SUBROUTINE GWF2AGO7FM(Iunitnwt, IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                       DELC
      USE GWFAGOMODULE, ONLY:NWELLS,WELL,PSIRAMP,TABROW,TABCOL,TABLAY, 
...
C3------RETURN
      RETURN
      END
      SUBROUTINE GWF2AGO7BD(KSTP,KPER,Iunitnwt,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                      VBVL,VBNM
      USE GWFAGOMODULE,ONLY:NWELLS,IWELCB,WELL,NWELVL,WELAUX,PSIRAMP,
...
C
      RETURN
      END
      SUBROUTINE SGWF2AGO7PNT(IGRID)
C  Change WEL data to a different grid.
      USE GWFAGOMODULE
C
        PSIRAMP=>GWFAGODAT(IGRID)%PSIRAMP
        IUNITRAMP=>GWFAGODAT(IGRID)%IUNITRAMP 
        NWELLS=>GWFAGODAT(IGRID)%NWELLS
        MXWELL=>GWFAGODAT(IGRID)%MXWELL
        NWELVL=>GWFAGODAT(IGRID)%NWELVL
        IWELCB=>GWFAGODAT(IGRID)%IWELCB
        IPRWEL=>GWFAGODAT(IGRID)%IPRWEL
        NPWEL=>GWFAGODAT(IGRID)%NPWEL
        IWELPB=>GWFAGODAT(IGRID)%IWELPB
        NNPWEL=>GWFAGODAT(IGRID)%NNPWEL
        WELAUX=>GWFAGODAT(IGRID)%WELAUX
        WELL=>GWFAGODAT(IGRID)%WELL
        NUMTAB=>GWFAGODAT(IGRID)%NUMTAB
        MAXVAL=>GWFAGODAT(IGRID)%MAXVAL
        TABTIME=>GWFAGODAT(IGRID)%TABTIME
        TABRATE=>GWFAGODAT(IGRID)%TABRATE
        TABVAL=>GWFAGODAT(IGRID)%TABVAL
        NUMSUP=>GWFAGODAT(IGRID)%NUMSUP
        NUMIRR=>GWFAGODAT(IGRID)%NUMIRR
        SFRSEG=>GWFAGODAT(IGRID)%SFRSEG
        UZFROW=>GWFAGODAT(IGRID)%UZFROW
        UZFCOL=>GWFAGODAT(IGRID)%UZFCOL
        UNITSUP=>GWFAGODAT(IGRID)%UNITSUP
        UNITIRR=>GWFAGODAT(IGRID)%UNITIRR
        SUPWEL=>GWFAGODAT(IGRID)%SUPWEL
        IRRWEL=>GWFAGODAT(IGRID)%IRRWEL
        NUMSEGS=>GWFAGODAT(IGRID)%NUMSEGS
        MAXSEGS=>GWFAGODAT(IGRID)%MAXSEGS
        MAXCELLS=>GWFAGODAT(IGRID)%MAXCELLS
        NUMCELLS=>GWFAGODAT(IGRID)%NUMCELLS
        WELLIRR=>GWFAGODAT(IGRID)%WELLIRR
        IRRFACT=>GWFAGODAT(IGRID)%IRRFACT
        IRRPCT=>GWFAGODAT(IGRID)%IRRPCT
C
      RETURN
      END
      SUBROUTINE SGWF2AGO7PSV(IGRID)
C  Save WEL data for a grid.
      USE GWFAGOMODULE
C 
        GWFAGODAT(IGRID)%PSIRAMP=>PSIRAMP
        GWFAGODAT(IGRID)%IUNITRAMP=>IUNITRAMP
        GWFAGODAT(IGRID)%NWELLS=>NWELLS
        GWFAGODAT(IGRID)%MXWELL=>MXWELL
        GWFAGODAT(IGRID)%NWELVL=>NWELVL
        GWFAGODAT(IGRID)%IWELCB=>IWELCB
        GWFAGODAT(IGRID)%IPRWEL=>IPRWEL
        GWFAGODAT(IGRID)%NPWEL=>NPWEL
        GWFAGODAT(IGRID)%IWELPB=>IWELPB
        GWFAGODAT(IGRID)%NNPWEL=>NNPWEL
        GWFAGODAT(IGRID)%WELAUX=>WELAUX
        GWFAGODAT(IGRID)%WELL=>WELL
        GWFAGODAT(IGRID)%NUMTAB=>NUMTAB
        GWFAGODAT(IGRID)%MAXVAL=>MAXVAL
        GWFAGODAT(IGRID)%TABTIME=>TABTIME
        GWFAGODAT(IGRID)%TABRATE=>TABRATE
        GWFAGODAT(IGRID)%TABVAL=>TABVAL
        GWFAGODAT(IGRID)%NUMSUP=>NUMSUP
        GWFAGODAT(IGRID)%NUMIRR=>NUMIRR
        GWFAGODAT(IGRID)%SFRSEG=>SFRSEG
        GWFAGODAT(IGRID)%UZFROW=>UZFROW
        GWFAGODAT(IGRID)%UZFCOL=>UZFCOL
        GWFAGODAT(IGRID)%UNITSUP=>UNITSUP
        GWFAGODAT(IGRID)%UNITIRR=>UNITIRR
        GWFAGODAT(IGRID)%SUPWEL=>SUPWEL
        GWFAGODAT(IGRID)%IRRWEL=>IRRWEL
        GWFAGODAT(IGRID)%NUMSEGS=>NUMSEGS
        GWFAGODAT(IGRID)%MAXSEGS=>MAXSEGS
        GWFAGODAT(IGRID)%MAXCELLS=>MAXCELLS
        GWFAGODAT(IGRID)%NUMCELLS=>NUMCELLS
        GWFAGODAT(IGRID)%WELLIRR=>WELLIRR
        GWFAGODAT(IGRID)%IRRFACT=>IRRFACT
        GWFAGODAT(IGRID)%IRRPCT=>IRRPCT
C
      RETURN
      END
