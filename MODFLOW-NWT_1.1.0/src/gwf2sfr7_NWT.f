...
C  (5) RUNOFF FROM UZF1 PACKAGE; (6) UNSATURATED FLOW BENEATH REACH (NOT AVAILABLE YET)
          IF(IUNIT(49).GT.0) THEN  !IUNIT(49): LMT
            ...
            IF(ITRFLG.EQ.1.AND.FLOWTYPE(1).EQ.'NA') THEN 
              ...
              FLOWTYPE(1)='STORAGE'
            ENDIF
            IF(SEG(5,nseg).NE.0.AND.FLOWTYPE(2).EQ.'NA') THEN  ! CHECK FOR SURFACE WATER PRECIP
              NFLOWTYPE = NFLOWTYPE + 1
              FLOWTYPE(2)='PRECIP'
              EXIT
            ENDIF
            IF(SEG(4,nseg).NE.0.AND.FLOWTYPE(3).EQ.'NA') THEN  ! CHECK FOR SURFACE WATER EVAP
              NFLOWTYPE = NFLOWTYPE + 1
              FLOWTYPE(3)='EVAP'
              EXIT
            ENDIF
...
C          CONDUCTANCE FOR EACH REACH.
            ...
            STRM(15, l) = hstr
            STRM(16, l) = cstr
      !if(l==136)write(229,*)istsg,kkiter,flowot
      !if(l==137)write(229,*)istsg,kkiter,flowin
      !if(l==247)write(229,*)istsg,kkiter,flowin 
...
C34-----STORE STREAM INFLOW, OUTFLOW, AND LEAKAGE FOR EACH REACH.
          ...
          STRM(18, l) = grad
          STRM(19, l) = h
      !if(l==136)write(229,*)'inbd',istsg,flowot
      !if(l==137)write(229,*)'inbd',istsg,flowin
      !if(l==247)write(229,*)'inbd',istsg,flowin 
C
